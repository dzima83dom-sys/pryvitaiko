...
function getAgeInstructions(ageGroup: ReturnType<typeof getAgeGroup>): string[] {
  switch (ageGroup) {
    ...
    case 'adult':
      return ['тримай баланс між теплом, повагою і природністю'];
    case 'elder':
      return [
...
function getRecipientRules(recipient: string): string[] {
  const r = normalize(recipient);

  const rules = [
    'адресат один, не кілька людей',
    'не перетворюй одного адресата на пару, сім’ю або групу людей',
    'не використовуй множину: "ви", "вас", "вам", якщо не вказано кількох адресатів',
  ];

  if (r.includes('мама')) {
    rules.push('адресат — тільки мама; не згадуй тата, батьків або обох разом');
  }

  if (r.includes('тато')) {
    rules.push('адресат — тільки тато; не згадуй маму, батьків або обох разом');
  }

  if (r.includes('брат')) {
    rules.push('адресат — тільки брат; не додавай сестру або інших родичів');
  }

  if (r.includes('сестра')) {
    rules.push('адресат — тільки сестра; не додавай брата або інших родичів');
  }

  if (r.includes('син')) {
    rules.push('адресат — тільки син; не додавай доньку або дітей у множині');
  }

  if (r.includes('донька')) {
    rules.push('адресат — тільки донька; не додавай сина або дітей у множині');
  }

  if (r.includes('бабуся')) {
    rules.push('адресат — тільки бабуся; не додавай дідуся або обох разом');
  }

  if (r.includes('дідусь')) {
    rules.push('адресат — тільки дідусь; не додавай бабусю або обох разом');
  }

  if (r.includes('хрещений')) {
    rules.push('адресат — тільки хрещений; не додавай хрещену');
  }

  if (r.includes('хрещена')) {
    rules.push('адресат — тільки хрещена; не додавай хрещеного');
  }

  return rules;
}

function getClassicWishRules(ageGroup: ReturnType<typeof getAgeGroup>): string[] {
  const rules = [
    'можна природно використовувати класичні побажання: міцного здоров’я, щастя, радості, тепла, добробуту, натхнення, гармонії',
    'класичні побажання дозволені, не прибирай їх повністю',
    'не будуй увесь текст тільки з шаблонних побажань',
  ];

  if (ageGroup === 'child') {
    rules.push('не додавай побажання мирного неба, якщо адресат дитина');
  } else {
    rules.push('доречно можна додати побажання мирного неба ближче до фіналу');
  }
...
function buildPrompt(state: IncomingState): string {
  ...
  const recipientRules = getRecipientRules(recipient);
  const classicWishRules = getClassicWishRules(ageGroup);
...
Правила щодо адресата:
${recipientRules.map((item) => `- ${item}`).join('\n')}

Правила щодо побажань:
${classicWishRules.map((item) => `- ${item}`).join('\n')}

Обов’язкові правила:
1. Текст має звучати природно, по-людськи, ніби його написала жива людина.
2. Не пиши шаблонно, але й не уникай повністю звичних теплих побажань, якщо вони доречні.
...
17. Не змішуй у зверненні або в тексті кількох родичів, якщо у вхідних даних вказаний один адресат.
18. Не пиши фрази на кшталт "тато і мама", "ви", "вас", якщо вибрано одного адресата.
19. Якщо адресат мама — пиши тільки для мами. Якщо адресат тато — пиши тільки для тата.
20. Якщо адресат не дитина, побажання мирного неба допустиме, але тільки природно і доречно.
21. Якщо адресат дитина, не додавай побажання мирного неба.
22. Побажання міцного здоров’я дозволене і доречне.
            ...
            content: [
              {
                type: 'input_text',
                text:
                  'Ти створюєш привітання українською мовою. Пиши природно, тепло, людяно, без канцеляриту. Адресат завжди один, якщо явно не сказано інше. Не змішуй маму і тата, не переходь на множину без потреби. Класичні побажання на кшталт здоров’я, щастя, тепла — допустимі, якщо вони доречні. Поверни тільки готовий текст.',
              },
            ],
          },
          {
