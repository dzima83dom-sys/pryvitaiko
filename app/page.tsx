...
import { telegramShareLink, viberShareLink } from '@/lib/share';
...
export default function HomePage() {
  ...
  const shareTelegram = () => {
    if (!store.generatedText) return;
    window.open(telegramShareLink(store.generatedText), '_blank');
  };

  const shareViber = () => {
    if (!store.generatedText) return;
    window.location.href = viberShareLink(store.generatedText);
  };
  ...
  if (!store.screen || store.screen === 'start') {
    return <StartScreen onStart={() => store.setScreen('recipient')} />;
  }

  if (store.screen === 'recipient') {
    return (
      <SelectionScreen
        step={1}
